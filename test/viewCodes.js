...
for(var code in unsupportedCodes){
  allCodes[code] = unsupportedCodes[code];
}

for(var code in supportedCodes){
  allCodes[code] = supportedCodes[code];
}
...
for (var code in allCodes) {
  ...
  codeSupport = {
    ...
    neighbourDelimitWorks: ("\u001b"+code+"a209ZELLO").match(ansiRegex()) === ("\u001b"+code).match(ansiRegex()),
  };
